...
优先级从高到低：
1. 本章计划中的必须保留、必须避免和作者临时要求。
2. 已确认的故事事实、人物状态、时间线与世界规则。
3. 近期焦点、长期作者意图和大纲。
4. 抽象文风特征。
...
返回严格 JSON：
{
  ...
  "timeline": [{"time":"故事内时间，没有则写相对时间","event":"可验证事件","participants":["亲历人物"],"location":"地点","causes":["直接前因"],"effects":["已发生后果"]}],
...
importance 为 1-5。evidence 必须是正文中逐字连续出现的短语；没有证据的字段不要输出。既有线索只更新，不得换名复制；只有正文已经兑现 expected_payoff 且满足 payoff_condition 才能 closed。description_updates 只记录具有辨识度、确实出现的短语，最多6条；普通动作不要记录。不要输出 Markdown。"""
...
INCUBATOR_PROMPT = """你是长篇小说的开书总导演。作者可能只给出一句灵感、一个人物、一种氛围或一个片段。
请把这颗种子发展成 2 套真正可以继续做全书规划的作品方案，而不是只给几句点子。
...
每套方案都必须包含可持续的主线、主要人物关系、世界边界、前期故事弧和结局方向。
...
返回严格 JSON：
{
  "options": [{
    "title": "作品名",
    "genre": "题材与类型标签",
    "positioning": "目标读者和阅读体验，80-150字",
    "premise": "完整核心构想，180-350字",
    "reader_promise": "读者持续能得到什么",
    "central_question": "全书最终回答的价值问题",
    "central_conflict": "可持续升级的贯穿冲突",
    "story_engine": "为什么能持续写很多章，必须具体",
    "outline": "从开篇到结局的全书大纲，700-1200字，包含前因、至少4个阶段、高潮与代价",
    "author_intent": "长期作者意图，150-300字",
    ...
    "first_arc": "前期故事弧的目标、阻力、转折与阶段结果，200-350字",
    "characters": [{
      "name": "姓名",
      "role": "身份和剧情功能",
      "aliases": ["常用称谓或别名"],
      "description": "不可变身份与背景",
      "personality": "稳定人格：思考、待人和压力反应",
      "appearance": "2-5个稳定外形锚点",
      "values": "价值观和底线",
      "fears": "恐惧、软肋与回避机制",
      "contradictions": "想要什么却因何抗拒",
      "relationships": "与其他主要人物的初始关系视角",
      "arc": "人物弧的起点、压力和可能变化",
      "hard_limits": "不可无依据改变的能力、认知或行为底线",
      "goal": "开篇目标",
      "state": "开篇身体/关系状态",
      "knowledge": "开篇已经知道的信息",
      "secrets": "本人隐瞒且他人不可自动知道的秘密",
      "voice": "词汇、句长、语气、避用词",
      "dialogue_examples": ["2条原创短对白，只展示声音与潜台词"]
    }],
    "world_entries": [{
      "title": "设定名",
      "category": "制度/地点/组织/历史/物件等",
      "keys": ["2-5个触发词"],
      "content": "可直接进入世界书的权威设定，80-180字",
      "canon": "hard或soft",
      "constant": false
    }]
  }]
...
每套方案提供 4-7 个主要人物、3-6 个必要世界设定。第一名人物是核心主角。人物卡要写可观察、可执行的差异，不用“复杂、善良、聪明”等空标签；对白示例不能复述大纲。target_chapters 要尊重作者选择的篇幅。
...
返回严格 JSON：
{
  ...
  "story_spine": "从开篇、发展、转折、高潮到结局的因果骨架，350-650字；详细分卷由下一阶段生成",
...
def _render_plan(current: dict[str, Any]) -> str:
    ...
    values = [
        f"目标：{plan.get('goal') or current.get('scene_goal', '')}",
        f"冲突：{plan.get('conflict', '')}",
        "必须保留：\n- " + "\n- ".join(plan.get("must_keep", [])),
        "必须避免：\n- " + "\n- ".join(plan.get("must_avoid", [])),
        f"转折：{plan.get('turning_point', '')}",
        f"结尾钩子：{plan.get('ending_hook', '')}",
...
def _character_core(character: dict[str, Any]) -> str:
    name = str(character.get("name", "")).strip()
    ledger = [
        item
    ...
    return (
        ...
        f"开篇/人工维护的知情边界：{character.get('knowledge', '')}\n"
        f"有来源的新增知情：{ledger_text}\n"
        f"隐瞒/秘密（其他角色不可自动知道）：{character.get('secrets', '')}\n"
...
def _character_context(
    ...
    # Keep the leading main cast as a safety net, then spend the remaining
    # slots on characters actually mentioned by the current route, memory or
    # recent prose. This prevents a large cast list from evicting the chapter
    # plan and continuation anchor.
    main_cast = {
        item[0]
        for item in rendered
        if "】｜main" in item[2]
    }
    main_cast.update(item[0] for item in rendered[:3])
    relevant = sorted(rendered, key=lambda item: (item[1], -item[0]), reverse=True)
    chosen = set(main_cast)
    for index, score, _ in relevant:
        if len(chosen) >= limit:
            break
        if score > 0:
            chosen.add(index)
    for index, _, _ in rendered:
        if len(chosen) >= limit:
            break
        chosen.add(index)
    return "\n\n".join(
        text for index, _, text in rendered if index in chosen
    )
...
def _fit_sections(
    ...
    return [section for section in sections if section.content.strip()], warnings
...
def build_prompt(project: dict[str, Any], request: dict[str, Any]) -> PromptBuild:
    ...
    characters = _character_context(project, query)
    ...
    task = (
        f"{mode_rules.get(mode, mode_rules['continue'])}\n"
        f"目标长度：约 {target_words} 个中文字符，允许上下浮动 20%。"
    )
    if instruction:
        task += f"\n作者本次要求：{instruction}"
    if selection:
    ...
    sections = [
        ...
        PromptSection(
            "样本文风约束",
            render_style_context(project.get("style", {}), query, recent_text),
            73,
            min_chars=500,
        ),
        PromptSection(
    ...
    requested_output = max(256, int(settings.get("max_tokens", 1800)))
